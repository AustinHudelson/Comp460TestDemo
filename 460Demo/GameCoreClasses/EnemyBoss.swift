...
@objc(EnemyBoss)
class EnemyBoss: Unit, PType
{
    required init(receivedData: Dictionary<String, AnyObject>){
        //Special case for sprite
        super.init(receivedData: receivedData)
        
        restoreProperties(EnemyBoss.self, receivedData: receivedData)
    ...
    override func addUnitToGameScene(gameScene: GameScene, pos: CGPoint) {
        ...
        if (self.isEnemy == true){
            if (Game.global.playerCount == 1){
                self.maxhealth.addModifier("1P", value: 0.4) //Sqrt of 1/3
                self.health = self.maxhealth.get()
                self.attackDamage.addModifier("1P", value: 0.3)
            } else if (Game.global.playerCount == 2){
                self.maxhealth.addModifier("2P", value: 0.6) //Sqrt of 2/3
                self.health = self.maxhealth.get()
                self.attackDamage.addModifier("2P", value: 0.6)
            }
        }
